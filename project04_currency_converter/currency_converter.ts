...
// define the interface or type of exchange rates

interface exchangeRateType {
    [currency : string] : {[currency : string] : number};
}

const exchangeRates : exchangeRateType = {
   "USD": {
...
async function currencyConverter(): Promise<void> {
 ...
 const fromCurrency = answers.from as keyof exchangeRateType;
 const toCurrency = answers.to as keyof exchangeRateType;
